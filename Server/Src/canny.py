...
def contourChannel(img,channel,color):
    ...
    chancontours,hier =  cv2.findContours(cv2.cvtColor(canny,cv2.COLOR_BGR2GRAY),cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
def clusterContours(img,contours):
    clusterRefs = [ClusterRef(cv2.minAreaRect(c),img[c[0][0][1],c[0][0][0]],c) for c in contours]
    clusternr = 0
    clusters = []
    for current in clusterRefs:
        intersections = []
        for r2 in clusterRefs:
            if(current == r2):
                continue
            if cv2.rotatedRectangleIntersection(current.rect,r2.rect) == cv2.INTERSECT_NONE:
                continue
        ...
        if len(intersections) == 0:
            ...
            for intersection in intersections:
                if intersection.clusterColor == current.clusterColor:
                    if intersection.clusterNumber == -1 and current.clusterNumber == -1:
                        ...
                        current.clusterNumber = clusternr
                        clusters.append(current.contours)
                        clusters[clusternr] = np.hstack([clusters[clusternr],intersection.contours])
                        clusternr = clusternr + 1
     ...
     #aanmekaarplakken                   clusters[intersection.clusterNumber] = np.hstack([clusters[intersection.clusterNumber],current.contours])
                    elif current.clusterNumber > -1 and intersection.clusterNumber == -1:
                        #new cluster gets merged with old cluster
                        intersection.clusterNumber = current.clusterNumber
                        clusters[current.clusterNumber] = np.hstack([clusters[current.clusterNumber],intersection.contours])
                    else:
                        #2clusters merged
                        clusters[current.clusterNumber] = np.hstack([clusters[current.clusterNumber],clusters[intersection.clusterNumber]])
            ...
            if not clustered:
                ...
                clusternr = clusternr + 1
                clusters.append(current.contours)
    clusters = clusters[c is not None]
    imgContours = np.zeros(img.size,dtype="uint8")
    cv2.drawContours(imgContours,c,-1,(125,125,125),1)
...
while(True):
    ...
    totalcanny =  merge2(redcanny,greencanny)#bluecanny,huecanny,satcanny,valcanny,hue2canny,sat2canny,lightcanny)
