...
namespace Jarvis.Library.Imaging
{
    ...
    public class UnmanagedImage : IDisposable
    {
        ...
        {
            // allocate memory for the image
            var returnVal = new byte[stride * height];
            fixed (byte* p = returnVal)
            {
                IntPtr ptr = (IntPtr)p;
                SystemTools.CopyUnmanagedMemory(ptr, imageData, stride * height);
            }
        ...
        /// 
        public IReadOnlyCollection<Point> CollectActivePixels( )
        {
        ...
        /// <returns>Returns list of points, which have other than black color.</returns>
        ///
        public IReadOnlyCollection<Point> CollectActivePixels( Rectangle rect )
        {
